...
function init () {
  ...
  socket = io(server, {origins:'localhost:* 192.168.*.*:* http://chrislhall.net:* http://www.chrislhall.net:*'})
  ...
  for (var loc in items) {
    ...
    if (Interactions.itemDefs[type].type === 'creature') { // && Math.random() < 0.2) {
      ...
      if (tileType && Interactions.tileDefs[tileType].action == 'sells') {
        money += Interactions.itemDefs[type].value
        setNewItem = true
...
function onChangeTile (data) {
  ...
  if (!(data.x === 0 && data.y >= 1 && data.y <= 5)) { // protected zone
    ...
    if (data.tileId === 0 && tiles[coord]) {
      ...
      var cost = Interactions.tileDefs[tiles[coord]].cost
      if (money >= cost) {
