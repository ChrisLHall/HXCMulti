function preload(){game.load.image("earth","assets/images/light_sand.png"),game.load.spritesheet("dude","assets/images/dude.png",64,64),game.load.spritesheet("enemy","assets/images/dude.png",64,64)}function create(){socket=io.connect(),game.world.setBounds(-500,-500,1e3,1e3),land=game.add.tileSprite(0,0,800,600,"earth"),land.fixedToCamera=!0;var a=Math.round(1e3*Math.random()-500),b=Math.round(1e3*Math.random()-500);player=game.add.sprite(a,b,"dude"),player.anchor.setTo(.5,.5),player.animations.add("move",[0,1,2,3,4,5,6,7],20,!0),player.animations.add("stop",[3],20,!0),player.body.maxVelocity.setTo(400,400),player.body.collideWorldBounds=!0,enemies=[],player.bringToTop(),game.camera.follow(player),game.camera.deadzone=new Phaser.Rectangle(150,150,500,300),game.camera.focusOnXY(0,0),cursors=game.input.keyboard.createCursorKeys(),setEventHandlers()}function onSocketConnected(){console.log("Connected to socket server"),socket.emit("new player",{x:player.x,y:player.y})}function onSocketDisconnect(){console.log("Disconnected from socket server")}function onNewPlayer(a){console.log("New player connected:",a.id),enemies.push(new RemotePlayer(a.id,game,player,a.x,a.y))}function onMovePlayer(a){var b=playerById(a.id);return b?(b.player.x=a.x,void(b.player.y=a.y)):void console.log("Player not found: ",a.id)}function onRemovePlayer(a){var b=playerById(a.id);return b?(b.player.kill(),void enemies.splice(enemies.indexOf(b),1)):void console.log("Player not found: ",a.id)}function update(){for(var a=0;a<enemies.length;a++)enemies[a].alive&&(enemies[a].update(),game.physics.collide(player,enemies[a].player));cursors.left.isDown?player.angle-=4:cursors.right.isDown&&(player.angle+=4),cursors.up.isDown?currentSpeed=300:currentSpeed>0&&(currentSpeed-=4),currentSpeed>0?(game.physics.velocityFromRotation(player.rotation,currentSpeed,player.body.velocity),player.animations.play("move")):player.animations.play("stop"),land.tilePosition.x=-game.camera.x,land.tilePosition.y=-game.camera.y,game.input.activePointer.isDown&&game.physics.distanceToPointer(player)>=10&&(currentSpeed=300,player.rotation=game.physics.angleToPointer(player)),socket.emit("move player",{x:player.x,y:player.y})}function render(){}function playerById(a){for(var b=0;b<enemies.length;b++)if(enemies[b].player.name===a)return enemies[b];return!1}var RemotePlayer=function(a,b,c,d,e){var f=d,g=e;this.game=b,this.health=3,this.player=c,this.alive=!0,this.player=b.add.sprite(f,g,"enemy"),this.player.animations.add("move",[0,1,2,3,4,5,6,7],20,!0),this.player.animations.add("stop",[3],20,!0),this.player.anchor.setTo(.5,.5),this.player.name=a.toString(),this.player.body.immovable=!0,this.player.body.collideWorldBounds=!0,this.player.angle=b.rnd.angle(),this.lastPosition={x:f,y:g}};RemotePlayer.prototype.update=function(){this.player.x!==this.lastPosition.x||this.player.y!==this.lastPosition.y?(this.player.play("move"),this.player.rotation=Math.PI+game.physics.angleToXY(this.player,this.lastPosition.x,this.lastPosition.y)):this.player.play("stop"),this.lastPosition.x=this.player.x,this.lastPosition.y=this.player.y},window.RemotePlayer=RemotePlayer;var game=new Phaser.Game(800,600,Phaser.AUTO,"",{preload:preload,create:create,update:update,render:render}),socket,land,player,enemies,currentSpeed=0,cursors,setEventHandlers=function(){socket.on("connect",onSocketConnected),socket.on("disconnect",onSocketDisconnect),socket.on("new player",onNewPlayer),socket.on("move player",onMovePlayer),socket.on("remove player",onRemovePlayer)};/*! Phaser v1.1.4 | (c) 2013 Photon Storm Ltd. */
